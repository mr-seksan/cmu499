        ...
        if (Array.isArray(data)) {
            data.forEach(item => {
                if (item.aod !== null && item.aod !== undefined) {
                    ...
                    if (aod >= 1 && aod < 25.99) {
                        item.remarken = 'Very Good';
                        item.remarkth = 'คุณภาพอากาศดีมาก';
                        item.color = '#00f2ff';

                    } else if (aod >= 26 && aod < 50) {
                        item.remarken = 'Good';
                        item.remarkth = 'คุณภาพอากาศดี';
                        item.color = '#38E54D';
...
const config_chart_day = async () => {
    ...
    let dateobj = { 'datestart': '2024-05-14', 'dateend': '2024-05-21' };
    ...
    try {
        ...
        let option = {
            ...
            visualMap: {
                ...
                pieces: [
                    { min: 1, max: 25, color: '#00f2ff' },
                    { min: 26, max: 50, color: '#38E54D' },
                    { min: 51, max: 100, color: '#FFDE4D' },
                    { min: 101, max: 200, color: '#FF8F00' },
                    { min: 200, color: '#FE0000' }
                ],
                outOfRange: {
                    color: '#fff'
                },
